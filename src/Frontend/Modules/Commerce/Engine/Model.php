...
class Model
{
    ...
    public static function getProductsByOrder($id)
    {
        ...
        foreach ($items as &$item) {
            // get image
            $img = FrontendModel::getContainer()->get('database')->getRecord('SELECT * FROM commerce_images WHERE product_id = ? ORDER BY sequence', array((int)$item['product_id']));
            if ($img) {
    ...
    public static function getAllByCategory($categoryId, $limit = 10, $offset = 0)
    {
        ...
        // prepare items for search
        foreach ($items as &$item) {
            $img = FrontendModel::getContainer()->get('database')->getRecord('SELECT * FROM commerce_images WHERE product_id = ? ORDER BY sequence', array((int)$item['id']));
            if ($img) {
    ...
    public static function getCategoriesTree($id = 0)
    {
        ...
        foreach ($items as &$row) {
            // set image path
            $img = FrontendModel::getContainer()->get('database')->getRecord('SELECT * FROM commerce_categories WHERE id = ?', array((int)$row['id']));
    ...
    public static function getOrder($id)
    {
        $item = (array)FrontendModel::getContainer()->get('database')->getRecord('SELECT c.*
			 FROM commerce_orders AS c', array($id), 'id');
    ...
    public static function insertComment(array $comment)
    {
        ...
        if ($comment['status'] == 'published') {
            ...
            // update num comments
            $db->update('commerce_products', array('num_comments' => $numComments), 'id = ?', $comment['product_id']);
        }
    ...
    public static function updateOrder($item, $orderId)
    {
        ...
        // update
        $db->update('commerce_orders', $item, 'id = ?', array((int)$orderId));
    }
    ...
    public static function updateOrderValue($item, $orderId, $productId)
    {
        ...
        // update
        $db->update('commerce_orders_values', $item, 'order_id = ? AND product_id = ?', array((int)$orderId, (int)$productId));
    }
    ...
    public static function deleteCompletedOrders()
    {
        ...
        // update record
        $db->delete('commerce_orders', 'status = ?', array('completed'));
    ...
    public static function deleteOrderValue($orderId, $productId)
    {
        ...
        // update record
        $db->delete('commerce_orders_values', 'order_id = ? AND product_id = ?', array((int)$orderId, (int)$productId));
