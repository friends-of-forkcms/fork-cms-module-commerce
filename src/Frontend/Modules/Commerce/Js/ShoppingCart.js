$(function () {
  $(".addToCart").click(function (e) {
    ...
    if (
      jsFrontend.data.exists("Commerce") &&
      jsFrontend.data.get("Commerce")["isProductDetail"]
    ) {
      var data = getFormData($("form[name=product]"), {
        "fork[module]": "Commerce",
        "fork[action]": "UpdateCart",
      });

      data["cartId"] = jsFrontend.data.get("Commerce.cartId");
      ...
      $.ajax({
        ...
        .done(function (response) {
          $(document).trigger({
            type: "commerce.product.add_to_cart",
            product: response.data.product,
          });

          $("[data-cart-total-quantity]").html(
        ...
        .fail(function (response) {
          $.each(
            response.responseJSON.data.errors.fields,
            function (key, value) {
              var field = $('[name*="[' + key + ']"'),
  ...
  $(".addToQuote").click(function (e) {
    ...
    // Some special stuff when we are on a product detail page
    if (jsFrontend.data.get("Commerce.isProductDetail")) {
