$(function () {
  $(".shopping-cart [data-product]").change(function () {
    ...
    if (jsFrontend.data.exists("Commerce.isQuote")) {
      data["quote"] = true;
    }
        ...
        if (data.cartValueId) {
          $(
            '[data-cart-value-id="' +
              ...
              productId +
              '"]'
          ).html("&euro; " + response.data.product.total);
          ...
          $('[data-total="' + productId + '"]').html(
            "&euro; " + response.data.product.total
          );
          // Set the options totals
          $.each(response.data.product.options, function (key, value) {
            $('[data-total="' + productId + "_option_" + key + '"]').html(
              "&euro; " + value
            );
          });
        }
  ...
  $("[data-remove-product]").click(function (e) {
    ...
    $.ajax({
      ...
      $(document).trigger({
        type: "commerce.product.remove_from_cart",
