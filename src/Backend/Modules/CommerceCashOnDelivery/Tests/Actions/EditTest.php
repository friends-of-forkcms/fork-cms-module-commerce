...
class EditTest extends BackendWebTestCase
{
    ...
    public function it_can_edit_the_payment_method(Client $client): void
    {
        ...
        self::assertPageLoadedCorrectly(
            ...
            ['form name="cash_on_delivery" method="post"']
        );
        ...
        $client->submit($form, [
            'cash_on_delivery[name]' => 'Cash On Delivery (edit)',
            'cash_on_delivery[orderInitId]' => $orderStatus->getId(),
            'cash_on_delivery[isEnabled]' => true,
        ]);
        ...
        self::assertResponseHasContent($client->getResponse(), 'Cash On Delivery (edit)');
        ...
        self::assertEquals('Cash On Delivery (edit)', $formValues['cash_on_delivery[name]']);
        self::assertEquals((string) $orderStatus->getId(), $formValues['cash_on_delivery[orderInitId]']);
        self::assertEquals('1', $formValues['cash_on_delivery[isEnabled]']);
    }
}
