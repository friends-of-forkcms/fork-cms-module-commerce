...
class DataGrid extends DataGridArray
{
    ...
    {
        $installedShipmentMethods = Model::get('catalog.repository.shipment_method')->findInstalledShipmentMethods(
        ...
        foreach ($shipmentMethods as $shipmentMethodName) {
            ...
            try {
                $infoXml = @new \SimpleXMLElement(
                    BACKEND_MODULES_PATH . '/Catalog/ShipmentMethods/' . $shipmentMethod['raw_name'] . '/Info.xml',
                    LIBXML_NOCDATA,
                    true
                );
    ...
    public static function getShipmentMethodsOnFilesystem(): array
    {
        ...
        $finder = new Finder();
        $directories = $finder->directories()->in(BACKEND_MODULES_PATH . '/Catalog/ShipmentMethods')->depth('==0');
