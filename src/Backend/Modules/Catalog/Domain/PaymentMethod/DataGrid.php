...
class DataGrid extends DataGridArray
{
    ...
    {
        $installedPaymentMethods = Model::get('catalog.repository.payment_method')->findInstalledPaymentMethods(
        ...
        foreach ($paymentMethods as $paymentMethodName) {
            ...
            try {
                $infoXml = @new \SimpleXMLElement(
                    BACKEND_MODULES_PATH . '/Catalog/PaymentMethods/' . $paymentMethod['raw_name'] . '/Info.xml',
                    LIBXML_NOCDATA,
                    true
                );
