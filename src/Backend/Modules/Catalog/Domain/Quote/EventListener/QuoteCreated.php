...
namespace Backend\Modules\Catalog\Domain\Quote\EventListener;

use Backend\Modules\Catalog\Domain\Cart\Cart;
use Backend\Modules\Catalog\Domain\Quote\Event\QuoteCreated as QuoteCreatedEvent;
...
final class QuoteCreated
{
    ...
    private function getCustomerMessage(QuoteDataTransferObject $quote, Cart $cart) : Swift_Mime_SimpleMessage {
        ...
        $message = Message::newInstance($subject)
                          ->parseHtml(
                              '/Catalog/Layout/Templates/Mails/Quote/ConfirmationCustomer.html.twig',
                              [
                                  'quote' => $quote,
                                  'cart' => $cart,
                              ],
    ...
    private function getCompanyMessage(QuoteDataTransferObject $quote, Cart $cart) : Swift_Mime_SimpleMessage {
        ...
        $message = Message::newInstance($subject)
                          ->parseHtml(
                              '/Catalog/Layout/Templates/Mails/Quote/ConfirmationCompany.html.twig',
                              [
                                  'quote' => $quote,
                                  'cart' => $cart,
                              ],
