...
class ConfirmOrder extends BaseConfirmOrder
{
    ...
    public function postPayment(): void
    {
        $query = $this->entityManager->getConnection()->prepare(
            ...
            FROM catalog_orders_buckaroo_payments
    ...
    {
        $query = $this->entityManager->getConnection()->prepare(
            'SELECT order_id, method, transaction_id FROM catalog_orders_buckaroo_payments WHERE order_id = :order_id'
        );
        $query->execute([
    ...
    private function createPayment(): StdClass
    {
        ...
        $this->entityManager->getConnection()
            ->insert('catalog_orders_buckaroo_payments', [
                'order_id' => $this->order->getId(),
                'method' => $this->option,
    ...
    {
        $this->entityManager->getConnection()->delete('catalog_orders_buckaroo_payments', [
            'order_id' => $this->order->getId(),
        ]);
