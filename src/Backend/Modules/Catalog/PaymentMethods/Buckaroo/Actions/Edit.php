...
namespace Backend\Modules\Catalog\PaymentMethods\Buckaroo\Actions;

use Backend\Core\Engine\Model;
use Backend\Modules\Catalog\PaymentMethods\Base\Edit as BaseEdit;
use Backend\Modules\Catalog\PaymentMethods\Buckaroo\BuckarooDataTransferObject;
use Backend\Modules\Catalog\PaymentMethods\Buckaroo\BuckarooType;
...
class Edit extends BaseEdit
{
    ...
    protected function install(): void
    {
        ...
        $query = $this->entityManager->getConnection()->prepare(
            'CREATE TABLE IF NOT EXISTS `catalog_orders_buckaroo_payments` (
    ...
    protected function uninstall(): void
    {
        ...
        $query = $this->entityManager->createNativeQuery(
            'SELECT COUNT(*) AS `count` FROM `catalog_orders_buckaroo_payments`',
            $rsm
        );
        ...
        if ((int)$result[0]['count'] === 0) {
            $this->entityManager->getConnection()
                ->prepare('DROP TABLE IF EXISTS `catalog_orders_buckaroo_payments`')
                ->execute();
        }
    }
